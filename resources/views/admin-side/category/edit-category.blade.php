...
<div class="flex flex-col justify-center items-center mt-4">
    ...
    <div class="flex items-center justify-between font-poppins mb-3 w-full md:w-4/5">
        <div class="text-left mb-2">
            <h1 class="text-2xl md:text-4xl font-extrabold leading-none text-black">
                EDIT <span class="text-black dark:text-black">CATEGORY</span>
            </h1>
        </div>
    ...
    <div class="w-full md:w-4/5 ">
        <form class="space-y-6" method="POST" action="/update-category" enctype="multipart/form-data" class="form-horizontal">
            @csrf
            <input type="hidden" value="{{$tbl_category->category_id}}" name="category_id">
            <div>
                <label for="category_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Category
                    Name</label>
                <input type="text" name="category_name" id="category_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Enter category name" value="{{ $tbl_category->category_name }}" required>
            </div>
